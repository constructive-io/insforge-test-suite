...
// Forward direction, deployed from the COMMITTED ejected module — not applied at
// runtime. `portability/materialized/packages/vendor-app-materialized` is an
// ordinary pgpm module (no `pgpm.apply.json`): the InsForge auth subsystem is
// already excluded and every reference is already rebound onto the generic
// provider in the committed SQL. Deploying it exercises the "materialize once,
// deploy plain" path — pgpm resolves its `requires` (auth-provider) and deploys
// that first, exactly like any hand-written module.
const roles = {
  anonymous: 'anon',
...
describe('committed forward (InsForge → pgpm) module on plain PostgreSQL', () => {
